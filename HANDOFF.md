...
## 下一步（接手的人從這裡開始）
1. Search Console 的 sitemap 那列誤填成 `/sitemap.xml`（開頭斜線讓它跑去抓網域根目錄的
   404），用無痕視窗重送、只填 `sitemap.xml`。**不急**：robots.txt 已有絕對網址的
   `Sitemap:` 宣告，Google 照樣找得到
...
- `edLinks()` 把 `work.links` 併進 `editions[0]` 且同平台不覆蓋——出新版卻沒建 edition 時，
  連結會被舊版蓋掉而永遠點不到（曾累積 77 個，已清空並落到所屬版本）。
  **新增版本一律連 edition 一起建，別只往 `work.links` 塞**
